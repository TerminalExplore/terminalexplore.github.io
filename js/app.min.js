$(function() {
    ...
    function shouldEnableLiteMode() {
        ...
        if (typeof URLSearchParams !== "undefined") {
            ...
            liteParam = searchParams.get("lite");
        }
    ...
    liteMode = shouldEnableLiteMode();
    window.siteLiteMode = liteMode;
    ...
    if (liteMode) {
        ...
        var staticGif = $gif.attr("data-static-src");
        if (staticGif) {
    ...
    $monitor.on("click", function() {
        ...
        $musicWidget.stop(true, true).fadeIn(500);
...
$(function() {
    ...
    var liteMode = !!window.siteLiteMode;
    ...
    function ensureSpotifyEmbedLoaded() {
        if (!$musicEmbed.length) {
            return;
        }

        if ($.trim($musicEmbed.attr("src")) !== "") {
            return;
        }

        var dataSrc = $.trim($musicEmbed.attr("data-src"));
        if (dataSrc) {
            $musicEmbed.attr("src", dataSrc);
        }
    }

    function closeMusicPanel() {
        $musicPanel.removeClass("open").attr("hidden", true);
        $musicToggle.attr("aria-expanded", "false");
    }

    function openMusicPanel() {
        ensureSpotifyEmbedLoaded();
        $musicPanel.removeAttr("hidden");
        window.requestAnimationFrame(function() {
            $musicPanel.addClass("open");
        });
        $musicToggle.attr("aria-expanded", "true");
    }
    ...
    $(document).on("click.music", function(event) {
        if ($(event.target).closest(".music-widget").length === 0) {
            closeMusicPanel();
        }
    });
    ...
    $shutdown.on("click", function() {
        ...
        setTimeout(function() {
            ...
            $musicWidget.stop(true, true).fadeOut(500);
